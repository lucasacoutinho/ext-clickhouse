...
{
    auto *intern =
        static_cast<php_clickhouse_client *>(zend_object_alloc(sizeof(php_clickhouse_client), ce));
...
static void apply_query_options(clickhouse::Query &q, zval *params, zval *settings)
{
    ...
    if (settings && Z_TYPE_P(settings) == IS_ARRAY) {
        ...
        ZEND_HASH_FOREACH_STR_KEY_VAL(ht, key, val)
        {
            ...
            q.SetSetting(std::string(ZSTR_VAL(key), ZSTR_LEN(key)), field);
            zend_string_release(str);
        }
        ZEND_HASH_FOREACH_END();
    }
...
static clickhouse::Query build_query(zend_string *query_str, zval *params, zval *settings,
    ...
    q.OnData([&](const clickhouse::Block &block) {
        ...
        for (size_t r = 0; r < rows; ++r) {
            ...
            for (size_t c = 0; c < cols; ++c) {
                ...
                add_assoc_zval_ex(&row, block.GetColumnName(c).c_str(),
...
ZEND_METHOD(ClickHouse_Driver_Client, selectByBlock)
{
    ...
    q.OnDataCancelable([&](const clickhouse::Block &block) -> bool {
    ...
    if (ZEND_FCI_INITIALIZED(fci_profile)) {
        q.OnProfile([&](const clickhouse::Profile &profile) {
            ...
            add_assoc_long(&arg, "rows_before_limit",
                           static_cast<zend_long>(profile.rows_before_limit));
            add_assoc_bool(&arg, "applied_limit", profile.applied_limit);
            add_assoc_bool(&arg, "calculated_rows_before_limit",
                           profile.calculated_rows_before_limit);
...
ZEND_METHOD(ClickHouse_Driver_Client, selectWithExternalData)
{
    ...
    ZEND_HASH_FOREACH_VAL(ht, entry)
    {
        ...
        tables.push_back(clickhouse::ExternalTable{
            std::string_view(Z_STRVAL_P(name_zv), Z_STRLEN_P(name_zv)), *block_intern->block});
    }
    ZEND_HASH_FOREACH_END();
...
ZEND_METHOD(ClickHouse_Driver_Client, getServerInfo)
{
    ...
    zend_update_property_stringl(clickhouse_ce_ServerInfo,
                                 PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "name",
                                 sizeof("name") - 1, info.name.c_str(), info.name.size());
    zend_update_property_stringl(
        clickhouse_ce_ServerInfo, PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "timezone",
        sizeof("timezone") - 1, info.timezone.c_str(), info.timezone.size());
    zend_update_property_stringl(
        clickhouse_ce_ServerInfo, PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "displayName",
        sizeof("displayName") - 1, info.display_name.c_str(), info.display_name.size());
    zend_update_property_long(
        clickhouse_ce_ServerInfo, PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "versionMajor",
        sizeof("versionMajor") - 1, static_cast<zend_long>(info.version_major));
    zend_update_property_long(
        clickhouse_ce_ServerInfo, PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "versionMinor",
        sizeof("versionMinor") - 1, static_cast<zend_long>(info.version_minor));
    zend_update_property_long(
        clickhouse_ce_ServerInfo, PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "versionPatch",
        sizeof("versionPatch") - 1, static_cast<zend_long>(info.version_patch));
    zend_update_property_long(clickhouse_ce_ServerInfo,
                              PHP_CLICKHOUSE_PROPERTY_OBJECT(return_value), "revision",
                              sizeof("revision") - 1, static_cast<zend_long>(info.revision));
    CLICKHOUSE_CATCH_RETURN
}

static const zend_function_entry class_ClickHouse_Driver_Client_methods[] = {
    ZEND_ME(
        ClickHouse_Driver_Client, __construct, arginfo_class_ClickHouse_Driver_Client___construct,
        ZEND_ACC_PUBLIC) ZEND_ME(ClickHouse_Driver_Client, execute,
                                 arginfo_class_ClickHouse_Driver_Client_execute, ZEND_ACC_PUBLIC)
        ZEND_ME(ClickHouse_Driver_Client, select, arginfo_class_ClickHouse_Driver_Client_select,
                ZEND_ACC_PUBLIC) ZEND_ME(ClickHouse_Driver_Client, selectByBlock,
                                         arginfo_class_ClickHouse_Driver_Client_selectByBlock,
                                         ZEND_ACC_PUBLIC)
            ZEND_ME(ClickHouse_Driver_Client, insert, arginfo_class_ClickHouse_Driver_Client_insert,
                    ZEND_ACC_PUBLIC)
                ZEND_ME(ClickHouse_Driver_Client, selectWithExternalData,
                        arginfo_class_ClickHouse_Driver_Client_selectWithExternalData,
                        ZEND_ACC_PUBLIC) ZEND_ME(ClickHouse_Driver_Client, ping,
                                                 arginfo_class_ClickHouse_Driver_Client_ping,
                                                 ZEND_ACC_PUBLIC)
                    ZEND_ME(ClickHouse_Driver_Client, resetConnection,
                            arginfo_class_ClickHouse_Driver_Client_resetConnection, ZEND_ACC_PUBLIC)
                        ZEND_ME(ClickHouse_Driver_Client, resetConnectionEndpoint,
                                arginfo_class_ClickHouse_Driver_Client_resetConnectionEndpoint,
                                ZEND_ACC_PUBLIC)
                            ZEND_ME(ClickHouse_Driver_Client, getCurrentEndpoint,
                                    arginfo_class_ClickHouse_Driver_Client_getCurrentEndpoint,
                                    ZEND_ACC_PUBLIC)
                                ZEND_ME(ClickHouse_Driver_Client, getServerInfo,
                                        arginfo_class_ClickHouse_Driver_Client_getServerInfo,
...
void php_clickhouse_register_client(int module_number)
{
    ...
    INIT_NS_CLASS_ENTRY(ce, "ClickHouse\\Driver", "Client", class_ClickHouse_Driver_Client_methods);
    ...
    memcpy(&clickhouse_client_handlers, zend_get_std_object_handlers(),
