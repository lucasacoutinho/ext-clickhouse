...
ZEND_METHOD(ClickHouse_Driver_Exception_ServerException, getClickHouseCode)
{
    ...
    zval *code_prop =
        zend_read_property(clickhouse_ce_ServerException, PHP_CLICKHOUSE_PROPERTY_OBJECT(ZEND_THIS),
...
static const zend_function_entry class_ClickHouse_Driver_Exception_ServerException_methods[] = {
    ZEND_ME(ClickHouse_Driver_Exception_ServerException, getClickHouseCode,
            arginfo_class_ClickHouse_Driver_Exception_ServerException_getClickHouseCode,
...
void php_clickhouse_register_exceptions(int module_number)
{
    ...
    clickhouse_ce_ConnectionException =
        zend_register_internal_class_ex(&ce, clickhouse_ce_ClickHouseException);
    ...
    clickhouse_ce_ServerException =
        zend_register_internal_class_ex(&ce, clickhouse_ce_ClickHouseException);
    zend_declare_property_long(clickhouse_ce_ServerException, "clickHouseCode",
                               sizeof("clickHouseCode") - 1, 0, ZEND_ACC_PROTECTED);

    /* ProtocolException extends ClickHouseException */
    INIT_NS_CLASS_ENTRY(ce, "ClickHouse\\Driver\\Exception", "ProtocolException", NULL);
    clickhouse_ce_ProtocolException =
        zend_register_internal_class_ex(&ce, clickhouse_ce_ClickHouseException);
...
{
    zend_object *exception_obj = zend_throw_exception(clickhouse_ce_ServerException, e.what(),
                                                      static_cast<zend_long>(e.GetCode()));

    if (exception_obj) {
#if PHP_VERSION_ID < 80000
        zval exception_zv;
        ZVAL_OBJ(&exception_zv, exception_obj);
        zend_update_property_long(clickhouse_ce_ServerException, &exception_zv, "clickHouseCode",
                                  sizeof("clickHouseCode") - 1,
                                  static_cast<zend_long>(e.GetCode()));
#else
        zend_update_property_long(clickhouse_ce_ServerException, exception_obj, "clickHouseCode",
                                  sizeof("clickHouseCode") - 1,
                                  static_cast<zend_long>(e.GetCode()));
#endif
    }
